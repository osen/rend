...
int main()
{
  std::cout << "Rend Level Test" << std::endl;

  SDL_Window* window = SDL_CreateWindow("Rend Level Test",
  ...
  {
    std::ifstream f("share/rend/samples/graveyard/graveyard.obj");
  ...
  while(running)
  {
    ...
    shader->setUniform("u_Model",
      rotate(glm::mat4(1.0f), radians(10.0f), vec3(1, 0, 0)) *
      rotate(glm::mat4(1.0f), radians(angle), vec3(0, 1, 0)) *
      translate(glm::mat4(1.0f), vec3(5, -2, 45))
    );
